...
moduleconfig = ['server_url','apikey', 'event_limit']
limit = 5      # Default
...
def handler(q=False):
    ...
    #Threat intelligence server url in TIServerUrl
    TIServerUrl= q["config"]["server_url"]
    #Key which is API key
    key = q["config"]["apikey"]
    ...
    if "ip-src" in q:
        if valid_ip(q["ip-src"]):
            #query to the server
            r["results"] += getIP(q["ip-src"], key, "ip", TIServerUrl)

    if "ip-dst" in q:
        if valid_ip(q["ip-dst"]):
            r["results"] += getIP(q["ip-dst"], key, "ip", TIServerUrl)

    if "domain" in q:
        r["results"] += getDomain(q["domain"], key, "domain", TIServerUrl)

    if 'hostname' in q:
        r["results"] += getDomain(q['hostname'], key, "domain", TIServerUrl)

    if 'md5' in q:
        r["results"] += getHash(q['md5'], key, "md5", TIServerUrl)
    if 'sha1' in q:
        r["results"] += getHash(q['sha1'], key,"sha1", TIServerUrl)
    if 'sha256' in q:
        r["results"] += getHash(q['sha256'], key, "sha256", TIServerUrl)
    if 'sha512' in q:
        r["results"] += getHash(q['sha512'], key, "sha512", TIServerUrl)
...
def getHash(hash, key,request_type, TIServerUrl):
    """
    ...
    :return:
    """
    enrichment_url = TIServerUrl + "/" + "api/v1/search/" + request_type+ "/" + hash
...
def getIP(ip, key, request_type, TIServerUrl):
    global limit
    toReturn = []
    #API access to the db is based on the IP address and specific url

    enrichment_url = TIServerUrl + "/" + "api/v1/search/" + request_type + "/" + ip
    Authorization_header = "Basic " + key
    headers = {'authorization':  Authorization_header}
...
def getDomain(domain, key, request_type, TIServerUrl):
    global limit
    toReturn = []

    enrichment_url = TIServerUrl + "/" + "api/v1/search/" + request_type+ "/" + domain
    Authorization_header = "Basic " + key
    headers = {'authorization': Authorization_header}
    req = requests.get(enrichment_url, headers=headers, verify=False)
